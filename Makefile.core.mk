...
ifeq ($(BUILD_FIPS), true)
	GOOS=$(GOOS_LOCAL) GOARCH=$(GOARCH_LOCAL) $(GO_BUILD_ENV_FIPS) LDFLAGS='$(RELEASE_LDFLAGS_FIPS)' common/scripts/gobuild.sh $(TARGET_OUT)/ $(STANDARD_BINARIES)
	GOOS=$(GOOS_LOCAL) GOARCH=$(GOARCH_LOCAL) $(GO_BUILD_ENV_FIPS) LDFLAGS='$(RELEASE_LDFLAGS_FIPS)' common/scripts/gobuild.sh $(TARGET_OUT)/ -tags=agent $(AGENT_BINARIES)
else
...
build-linux: depend
ifeq ($(BUILD_FIPS), true)
	GOOS=linux GOARCH=$(GOARCH_LOCAL) $(GO_BUILD_ENV_FIPS) LDFLAGS='$(RELEASE_LDFLAGS_FIPS)' common/scripts/gobuild.sh $(TARGET_OUT_LINUX)/ $(STANDARD_BINARIES)
	GOOS=linux GOARCH=$(GOARCH_LOCAL) $(GO_BUILD_ENV_FIPS) LDFLAGS='$(RELEASE_LDFLAGS_FIPS)' common/scripts/gobuild.sh $(TARGET_OUT_LINUX)/ -tags=agent $(LINUX_AGENT_BINARIES)
else
